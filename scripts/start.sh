...
chmod +x "$ROOT/scripts/poll.sh" "$ROOT/scripts/poll-copilot.sh"
...
load_agent "${LAUNCHD_PREFIX}-copilot" "$AGENTS/${LAUNCHD_PREFIX}-copilot.plist"

echo ""
PORT=$(grep "^port:" "$ROOT/config.yaml" 2>/dev/null | awk -F': *' '{print $2}')
PORT=${PORT:-666}
echo "Dashboard at http://localhost:$PORT"
echo "Logs: /tmp/dashboard-server.log  /tmp/dashboard-poll.log  /tmp/dashboard-copilot.log"
